...
function odata_mimir_compare_order_values(mixed $left, mixed $right): int
{
    $leftNumeric = is_int($left) || is_float($left) || (is_string($left) && is_numeric($left));
    $rightNumeric = is_int($right) || is_float($right) || (is_string($right) && is_numeric($right));
    if ($leftNumeric && $rightNumeric) {
        return ((float) $left) <=> ((float) $right);
    }

    $leftText = is_scalar($left) ? trim((string) $left) : '';
    $rightText = is_scalar($right) ? trim((string) $right) : '';
    return strcmp($leftText, $rightText);
}

/**
 * Mímir query.php kent geen $orderby. Ratatoskr gebruikt die clausule
 * (o.a. nieuwste ontvangstdatum), dus die sortering gebeurt lokaal.
 *
 * @param list<array<string, mixed>> $rows
 * @return list<array<string, mixed>>
 */
function odata_mimir_apply_orderby(array $rows, string $orderby): array
{
    ...
    foreach (explode(',', $orderby) as $part) {
        ...
        $field = trim((string) ($bits[0] ?? ''));
        if ($field === '') {
            continue;
        }
        $direction = strtolower((string) ($bits[1] ?? 'asc'));
        $specs[] = [
            'field' => $field,
            'desc' => $direction === 'desc',
        ];
    ...
    usort($rows, static function ($left, $right) use ($specs): int {
        if (!is_array($left) || !is_array($right)) {
            return 0;
        }
        foreach ($specs as $spec) {
            $field = (string) $spec['field'];
            $cmp = odata_mimir_compare_order_values($left[$field] ?? null, $right[$field] ?? null);
            if ($cmp !== 0) {
                return !empty($spec['desc']) ? -$cmp : $cmp;
            }
        }
        return 0;
    });
 ...
 * $odataQuery gebruikt Ratatoskr-keys zoals $select / $filter.
 *
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Ratatoskr-filecache wordt overgeslagen.
